...
var CppModuleGenerator = module.exports = function CppModuleGenerator(
  ...
  // Debug aliases.
  grunt_module.configure("fake", names.debug, {
    configuration: "Debug",
    cwd: opts.path
  });
  grunt_module.aliasMore("debug", "debug:" + opts.name);
  grunt_module.alias("debug:" + opts.name, "fake:" + names.debug);
  ...
  // Release aliases.
  grunt_module.configure("fake", names.release, {
    configuration: "Release",
    cwd: opts.path
  });
  grunt_module.aliasMore("release", "release:" + opts.name);
  grunt_module.alias("release:" + opts.name, "fake:" + names.release);


  // Test aliases.
  grunt_module.configure("fake", names.test, {
    configuration: "Test",
    cwd: opts.path
  });
